...
export default function SupplierPortal() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ws = new WebSocket('ws://localhost:8000/ws/supplier');
  ...
  const handleOrderResponse = async (po_number, action) => {
    try {
      ...
      await fetch(`http://localhost:8000${endpoint}`, {
        ...
        body: JSON.stringify({ po_number: po_number, confirmed_delivery: confirmedDate })
      });

      // Purge the processed order from local UI state
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="flex justify-between items-end">
                    <div>
                      ...
                      <div className="text-xl font-black text-slate-800">{order?.quantity || 0}kg {order?.item || 'Item'}</div>
                    </div>
                  ...
                  <div className="pt-2">
                    <label className="text-xs font-bold text-slate-500 mb-1 flex items-center gap-1"><Calendar className="w-3 h-3" /> Target Delivery Date</label>
                    <input 
                      type="date" 
