...
import { Package, Check, X, Truck, Building2 } from 'lucide-react';
...
export default function SupplierPortal() {
  ...
  useEffect(() => {
    // Dedicated connection for the supplier routing tier
    const ws = new WebSocket('ws://localhost:8000/ws/supplier');
  ...
  const handleOrderResponse = async (po_number, action) => {
    try {
      ...
      // Calculate a mock confirmation date (24 hours from now)
      ...
      await fetch(`http://localhost:8000${endpoint}`, {
        ...
        body: JSON.stringify({
          po_number: po_number,
          confirmed_delivery: deliveryDate
        })
      });

      // Purge the processed order from the local UI state
      setOrders(orders.filter(order => order.po_number !== po_number));
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto mb-8 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">B2B Order Management System</p>
          </div>
        </div>
            ...
            {orders.map((order) => (
              <div key={order.po_number} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="border-b border-slate-100 bg-slate-50 p-4 flex justify-between items-center">
                  <span className="font-mono text-xs font-bold text-slate-500">{order.po_number}</span>
                  <span className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs font-bold rounded uppercase tracking-wider">Requires Action</span>
                </div>
                
                <div className="p-5 space-y-4">
                  <div>
                    <div className="text-sm text-slate-500 font-medium mb-1">Client</div>
                    <div className="font-bold text-slate-900">{order.from_store}</div>
                  </div>
                  
                  <div className="flex justify-between items-end">
                    <div>
                      <div className="text-sm text-slate-500 font-medium mb-1">Requested Item</div>
                      <div className="text-xl font-black text-slate-800">{order.quantity}kg {order.item}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm text-slate-500 font-medium mb-1">Total Value</div>
                      <div className="text-lg font-bold text-emerald-600">₹{order.total_cost}</div>
                    </div>
                  </div>
