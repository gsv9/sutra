...
async def approve_procurement(data: dict):
    ...
    `data` must be the full recommendation dict broadcast earlier,
    including po_number, item, quantity, supplier, unit_price,
    total_cost, savings, reasoning, expected_delivery_days.
    """
...
def reject_procurement(data: dict):
    ...
    `data` must be the full recommendation dict, same as approve.
    """
    rejected_po = procurement_engine.handle_rejection(
        data,
...
def get_orders():
    """Returns all procurement orders (pending, approved, rejected, confirmed — everything)."""
    db = SessionLocal()
    try:
...
async def supplier_confirm(data: dict):
    ...
    Creates a new 'confirmed' row referencing the approved order.
    `data` must include po_number and the order's original fields.
    """
    print(f"[API] Supplier confirmed order: {data['po_number']}")

    original_order = procurement_engine.get_order(data["po_number"])
    if not original_order:
        return {"status": "error", "message": "Original order not found"}
