...
def real_phi3_inference(context):
    """
    Calls Member 2's Phi-3 Mini inference pipeline.
    His ml/ folder is a package with relative imports.
    Must be imported as 'ml.agent' not just 'agent'.

    NOTE: on the integration PC this points to ml_test.
    Change back to 'from ml.agent import run_inference'
    on the Snapdragon demo machine.
    """
    import sys
    import os

    sutra_root = os.path.abspath(
        os.path.join(os.path.dirname(__file__), '..')
    )
    if sutra_root not in sys.path:
        sys.path.insert(0, sutra_root)

    from ml_test.agent import run_inference
...
def handle_arduino_event(event):
    ...
    # Step 3: Save this recommendation as a new pending row
    # Every recommendation gets recorded — nothing overwritten later
    ...
    # Step 4: Package result with original event
    result = {
        "po_number": po_number,
        "event": event,
        "context": context,
    ...
    # Step 5: Return result — main.py handles broadcasting
    return result